...
- 内置适配器已经实现 Skill 包解析。`SKILL.md` 是 primary source file，包内成员写入 `Asset.sourceFiles`，工具原生身份写入 `Asset.nativeIdentity`。
- 二进制 support 文件会进入包 hash 和增量失效判断。第一阶段内置转换不会部署二进制 support 文件；省略这些文件会让转换标记为 `partial`，并给出明确 warning。
- 文本 support 文件在 Skill 迁移中会输出为 `copy` source output；生成的 `SKILL.md` 仍然是语义文本输出。
- support 或 metadata 文件变化会让增量扫描重新解析所属 `SKILL.md`；support 文件上的诊断会归属到所属 Skill 资产。
- 已实现 native field diagnostics 和 partial conversion warnings，包括不支持的 Skill/Agent/Rule/MCP 字段、目标工具 Skill 命名规则、目标必需 description、Cursor rule 激活元数据，以及无法在目标格式表达的 MCP 字段。

仍然保留的第一阶段限制：二进制 support-file migration 尚未实现；Rule 层级语义和 MCP scope 迁移仍窄于原生工具；Skill 到 agent/MCP 等跨资源依赖还需要后续 package-level planning。
