...
命令在传输层命名为 `ai-config-hub:v1:<command>`，例如 `ai-config-hub:v1:scan.start`。调用请求包含 `apiVersion: 1`。命令名、`type`/`status`/`phase` 等封闭判别值在同一主版本内不得新增、删除或改变语义；新增判别值需要发布 v2。只有 Schema 明确标为开放的 `extensions` 命名空间或明确声明可扩展的可选字段，才能在 v1 中新增键。

破坏性变更发布新命令主版本，并在至少一个桌面应用发布周期内保留旧 handler。客户端只忽略 Schema 明确允许扩展的未知可选字段；未知状态、phase 或事件 `type` 视为契约版本错误并停止写操作。遇到不支持的主版本返回 `API_VERSION_UNSUPPORTED`，不得尝试降级写入。
...
`migration.preview` 只返回不可变 `planId`、`planHash`、差异和哈希，不返回任何执行凭据。Electron main process 必须通过原生确认 UI 展示从持久计划重新读取的目标、差异、兼容警告和漂移状态；只有真实用户确认后，main 才在自身内存创建单次、短时 `ConfirmationGrant`，绑定 `action = "deployment.execute"`、`planId`、`planHash`、窗口会话和过期时间。renderer 随后的 `deployment.execute` 只传 `planId`；main 重新读取计划并消费匹配 grant。grant 不序列化、不写 SQLite/日志、不经 IPC，也不能被 renderer 获取、重放或构造。

CLI 在命令边界展示同一计划；交互确认，或自动化明确提供 `--yes --plan <planId>` 后，CLI 进程才创建同样的进程内单次 grant并直接调用核心用例。非交互模式缺少显式 `--yes` 必须失败。回滚采用相同模式：调用者只传 `deploymentId`，desktop main/CLI 在自身边界确认后创建绑定回滚计划哈希的内存 grant。

### 4.2 跨进程部署协调

desktop 和 CLI 通过共享 SQLite `deployment_locks` 按 `canonicalTargetKey` 获取 lease 与单调 fencing token。获取、续租和释放使用 `BEGIN IMMEDIATE` 短事务；每次备份、写入、替换和补偿前都验证 owner、lease expiry 与 fence。进程内 mutex 只能优化竞争。该机制不能约束编辑器或其他外部程序，因此实际文件操作仍须复核预览哈希、当前哈希和 file identity。
...
```ts
type TaskKind = "scan" | "deployment" | "rollback";
type TaskPhase =
  | "queued" | "discovering" | "reading" | "parsing" | "validating"
  | "committing" | "preflight" | "backing_up" | "writing"
  | "restoring" | "verifying" | "rolling_back" | "completed";
type TaskTerminalStatus =
  | "succeeded" | "partially_succeeded" | "cancelled"
  | "failed" | "rolled_back";
type JsonValue =
  | null | boolean | number | string
  | JsonValue[] | { [key: string]: JsonValue };

type OrderedEventBase = {
  apiVersion: 1;
  eventVersion: 1;
  taskId: string;
  sequence: number;
...
type AcceptedEvent = OrderedEventBase & {
  type: "accepted";
  payload: { taskKind: TaskKind; phase: "queued"; acceptedAt: string };
};
type PhaseChangedEvent = OrderedEventBase & {
  type: "phase.changed";
  payload: { from: TaskPhase | null; to: TaskPhase };
};
type ProgressEvent = OrderedEventBase & {
  type: "progress";
  payload: {
    phase: TaskPhase;
    completed: number;
...
type ItemFailedEvent = OrderedEventBase & {
  type: "item.failed";
  payload: {
    itemRef: string;
    diagnosticId: string;
...
type CancelRequestedEvent = OrderedEventBase & {
  type: "cancel.requested";
  payload: { reason: "user" | "shutdown"; effectiveAfterPhase: TaskPhase };
};
type CompletedEvent = OrderedEventBase & {
  type: "completed";
  payload: {
    ...
    skippedCount: number;
    resultRef?: string;
    systemRecoveryLock: boolean;
  };
};

type CursorResetEvent = {
  apiVersion: 1;
  eventVersion: 1;
  taskId: string;
  sequence: null;
...
type TaskEvent =
  | AcceptedEvent | PhaseChangedEvent | ProgressEvent | ItemFailedEvent
  | CancelRequestedEvent | CompletedEvent | CursorResetEvent | SnapshotEvent;
```

每个 `type` 对应固定 payload，不能用 `Record<string, unknown>` 绕过契约。`type`、`status`、`phase` 是 v1 封闭判别值；需要业务扩展的事件只能在明确声明的可选 `extensions?: Record<string, JsonValue>` 字段中增加 namespaced 数据，且不得改变核心处理语义。
...
- 主进程持久化任务状态、计数、最终结果和有限事件尾部。若 `afterSequence` 小于 `earliestAvailableSequence - 1`，游标已过期：服务端先发送非持久化 `cursor.reset(sequence=null)`，再发送独立 `snapshot(sequence=null)`。客户端必须丢弃旧增量视图，以 snapshot 原子替换本地状态，将游标设置为 `snapshot.payload.lastSequence`，再接收更大的有序事件。`snapshot` 不是 `phase.changed`，也不占用或伪造持久事件 sequence。
...
`phase` 是运行进度，不是领域状态。`ScanRun.status` 只使用 `queued`、`detecting`、`discovering`、`parsing`、`resolving`、`diagnosing`、`indexing`、`succeeded`、`partially_succeeded`、`failed`、`cancelled`；`DeploymentRecord.status` 只使用领域模型规定的 `planned`、`confirmed`、`backed_up`、`writing`、`verifying`、`succeeded`、`failed`、`rolling_back`、`rolled_back`。补偿无法自动完成时仍记录 `DeploymentRecord.status = "failed"`，另由系统 recovery lock/mode 表示 `recovery_required`，不能把它写成 DeploymentRecord 状态。
...
单个文件解析失败或单个不相关根无权限可产生扫描终态 `partially_succeeded`。`partial` 只用于 `ConversionResult.level`，不得作为 `ScanRun.status`。最终数据必须同时给出 `succeededCount`、`failedCount`、`skippedCount` 和失败项的稳定诊断 ID。任何可能造成目标文件不一致的错误都不是部分成功：部署必须回滚；无法自动恢复时记录 `DeploymentRecord.status = "failed"` 并启用系统 recovery lock/mode。
...
| 事件 | `accepted`；各扫描 phase 的 `phase.changed`/`progress`；逐项 `item.failed`；最终 `completed`，状态为 `succeeded`、`partially_succeeded`、`cancelled` 或 `failed` |
...
| 权限边界 | 只读磁盘；目标必须在已授权且适配器声明的配置根；计划本身不授权执行，确认后产生的 `ConfirmationGrant` 只存在于 desktop main 或 CLI 进程内 |
...
| 错误码 | `VALIDATION_FAILED`、`NOT_FOUND`、`CONFIRMATION_REQUIRED`、`CONFIRMATION_EXPIRED`、`STALE_PREVIEW`、`TARGET_LOCKED`、`FENCE_REJECTED`、`READ_ONLY_RECOVERY`、`PERMISSION_DENIED` |
| 权限边界 | 唯一常规文件写入口；main 从持久计划重建内容，消费内部绑定 `planId + planHash` 的单次 `ConfirmationGrant`，并复核 canonical path、symlink、源/目标哈希、file identity、SQLite lease/fence 和写权限 |
...
| 请求 | `{ deploymentId: string }`；回滚确认 grant 由 desktop main/CLI 边界创建并仅在进程内消费 |
| 响应 | `{ taskId: string, rollbackId: string, status: "queued", acceptedAt: string }` |
| 错误码 | `VALIDATION_FAILED`、`NOT_FOUND`、`CONFIRMATION_REQUIRED`、`BACKUP_MISSING`、`BACKUP_HASH_MISMATCH`、`STALE_TARGET`、`TARGET_LOCKED`、`FENCE_REJECTED`、`READ_ONLY_RECOVERY`、`PERMISSION_DENIED` |
...
```ts
type CliMeta = { generatedAt: string; partialSuccess: boolean };
...
```json
{"schemaVersion":1,"command":"scan","ok":true,"data":{"taskId":"scn_01JZ8M2Y6S7Q","status":"partially_succeeded","phase":"completed","counts":{"discovered":42,"succeeded":40,"failed":2,"skipped":0},"diagnosticIds":["diag_01JZ8M31","diag_01JZ8M32"]},"meta":{"generatedAt":"2026-06-21T08:00:00.000Z","partialSuccess":true}}
```
...
`planId` 和 `planHash` 只标识不可变计划，不是执行授权。CLI 不写出、持久化或传递执行凭据；确认后创建的 `ConfirmationGrant` 只存在于当前 CLI 进程内。

### 7.6 `deploy`

映射：`deployment.execute`。交互终端显示差异摘要并要求输入确认；非交互模式必须显式传 `--yes --plan <planId>`，且计划仍需有效。确认发生在 CLI 边界，执行调用只使用 `planId` 和进程内 grant。
...
```json
{"schemaVersion":1,"command":"history","ok":true,"data":{"items":[{"id":"rbk_01JZ8S1","kind":"rollback","status":"succeeded","projectId":"prj_01JZ8","createdAt":"2026-06-21T08:06:00.000Z","relatedId":"dep_01JZ8R2"},{"id":"dep_01JZ8R2","kind":"deployment","status":"succeeded","projectId":"prj_01JZ8","createdAt":"2026-06-21T08:05:00.000Z","relatedId":"pln_01JZ8Q5"}],"nextCursor":null},"meta":{"generatedAt":"2026-06-21T08:07:00.000Z","partialSuccess":false}}
```
...
- 漂移测试必须证明过期 `planId`、改变的源哈希、改变的目标哈希、失效 lease 和旧 fencing token 都无法触发写入。
- `--json` 黄金测试固定判别联合、字段类型和互斥关系；只有 Schema 明确开放的可选扩展字段允许新增，任何日志或进度文本污染 `stdout` 都应失败。
