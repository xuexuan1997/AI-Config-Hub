...
- 诊断、扫描任务、用户设置、部署历史、操作日志、配置备份和数据库备份元数据；
...
`tools`、`projects`、`scopes`、`assets`、`asset_references` 和派生 `diagnostics` 可由事实来源文件重建。`settings`、`scan_runs`、`deployments`、`deployment_operations`、配置 `backups`、`database_backups`、`recovery_locks` 与 `schema_migrations` 是非派生记录，不能用重扫冒充恢复；它们必须纳入一致数据库备份。`deployment_locks` 只是可过期的协调状态，可以在确认无活动 owner 后清理重建。
...
所有业务表启用外键约束。MVP 完整逻辑表清单为：`tools`、`projects`、`scopes`、`assets`、`asset_references`、`diagnostics`、`settings`、`scan_runs`、`deployments`、`deployment_operations`、`backups`、`database_backups`、`deployment_locks`、`recovery_locks`、`schema_migrations`。实现可增加内部 staging 表和生成列，但不得改变所列数据所有权、唯一性和删除语义。
...
| --- | --- |
| 主键 | `id TEXT`，随机 surrogate UUID |
| 核心字段 | `setting_key`、`value_json`、`visibility`（`public`/`privileged`）、`revision`、`created_at`、`updated_at` |
| 唯一约束 | `UNIQUE(setting_key)`；`revision >= 1` |
| 关键索引 | `idx_settings_visibility(visibility, setting_key)`、`idx_settings_updated(updated_at DESC)` |

`settings` 是不能从配置文件重建的非派生用户状态，必须进入数据库备份。`value_json` 只允许产品注册的公开或特权设置 Schema，不保存秘密、任意命令、环境变量值或未经授权的文件路径。
...
### 3.12 `database_backups`

| 项目 | 定义 |
| --- | --- |
| 主键 | `id TEXT`，随机 surrogate UUID |
| 核心字段 | `domain_id`（稳定 `DatabaseBackupId`/`backupId`）、`reason`、`state`、`backup_path_normalized`、`manifest_version`、`manifest_hash`、`database_file_hash`、`database_schema_version`、`source_database_id`、`size_bytes`、`created_at`、`verified_at`、`expires_at`、`failure_code` |
| 唯一约束 | `UNIQUE(domain_id)`、`UNIQUE(backup_path_normalized)` |
| 关键索引 | `idx_database_backups_retention(state, expires_at)`、`idx_database_backups_schema(database_schema_version, created_at DESC)`、`idx_database_backups_reason(reason, created_at DESC)` |

该表只索引 SQLite 一致快照，不关联 `deployment_id`，也不复用配置 `backups`。数据库备份是非派生恢复事实；只有 Online Backup API 或安全等价机制生成、manifest/hash/integrity/权限全部验证通过的记录才能进入 `state = 'verified'` 并被恢复使用。

### 3.13 `deployment_locks`
...
desktop 与 CLI 共用该表协调正常部署。获取、续租和释放均在短 `BEGIN IMMEDIATE` 事务中完成：获取前必须确认相同 `canonical_target_key` 不存在 unresolved `recovery_locks`；随后才可插入空键、由当前 owner 续租，或接管已过期租约。每次新获取/接管都将 `fence_token` 单调递增且永不复用。释放使用 `(canonical_target_key, owner_id, fence_token)` 条件删除，旧 owner 不能删除新租约。进程内 mutex 只能减少本进程竞争，是优化而不是正确性边界。

### 3.14 `recovery_locks`

| 项目 | 定义 |
| --- | --- |
| 主键 | `canonical_target_key TEXT`，与部署 canonical 目标集合使用完全相同的规范化算法 |
| 外键 | `deployment_id → deployments.id ON DELETE RESTRICT`，保留触发人工恢复的审计链 |
| 核心字段 | `reason`、`created_at`、`resolved_at`、`resolution_evidence_json`、`recovery_owner_id`、`recovery_claim_expires_at`、`recovery_fence_token INTEGER` |
| 约束 | `resolved_at IS NULL` 时 `resolution_evidence_json` 必须为空；解除时二者必须同时存在；`recovery_fence_token` 单调递增 |
| 关键索引 | `idx_recovery_locks_unresolved(resolved_at, created_at)`、`idx_recovery_locks_deployment(deployment_id)`、`idx_recovery_locks_claim(recovery_claim_expires_at)` |

`recovery_locks` 是持久的系统安全状态，不是 `DeploymentRecord.status`，也不随 `deployment_locks` lease 到期或释放而消失。任何 unresolved 行都阻止同一 key 获取正常 deployment lease。受控恢复用例只能在 `BEGIN IMMEDIATE` 中临时 claim 该 recovery row，使用其单调 `recovery_fence_token` 执行恢复；只有目标哈希、文件 identity、配置语义、备份和全量验证全部通过后，才可显式写入 `resolved_at` 与脱敏 `resolution_evidence_json`。删除历史 deployment 前必须先保留或解决其 recovery lock。

### 3.15 `schema_migrations`

| 项目 | 定义 |
| --- | --- |
| 主键 | `version INTEGER`，单调递增 |
| 外键 | `pre_migration_backup_id → database_backups.id ON DELETE RESTRICT`；初始空库创建可为空，现有库升级必须非空 |
| 核心字段 | `name`、`checksum`、`started_at`、`applied_at`、`state`、`error_code`、`app_version`、`pre_migration_backup_id` |
| 唯一约束 | `UNIQUE(name)`、`UNIQUE(checksum)` 仅用于已应用迁移；同名迁移校验和改变必须拒绝启动写模式 |
| 关键索引 | `idx_schema_migrations_state(state, version DESC)`、`idx_schema_migrations_backup(pre_migration_backup_id)` |

迁移记录通过受约束外键精确关联迁移前数据库备份，使恢复流程可由 migration version 定位 `backupId`，不能靠目录名或时间戳猜测。实现迁移元数据与备份目录的最低恢复读取路径必须独立于可派生业务索引。
...
WAL 允许 UI/CLI 读查询与扫描写事务并发，但不代表允许多个写任务修改相同目标。desktop 与 CLI 可能是独立进程，必须以共享 SQLite `deployment_locks` 租约和 fencing token 协调，并在获取 lease 前检查持久 `recovery_locks`；进程内互斥锁只用于降低本进程竞争。应用仍对扫描提交、迁移和部署历史更新使用短写事务。定期 checkpoint 在空闲期执行，不能在部署原子写入窗口中强制阻塞。
...
2. 创建 `database_backups(state = 'creating')` 记录，使用 SQLite Online Backup API（或驱动提供的等价一致性 backup API）生成数据库快照，验证 manifest、文件哈希、权限、`integrity_check` 和 Schema 后更新为 `verified`。不能直接复制活动中的 DB/WAL/SHM 文件。若运行环境确实不提供安全 backup API，只允许在取得跨进程独占锁、停止全部写入、完成并校验 checkpoint 后复制关闭状态的数据库；复制后必须对源和副本执行完整性与 Schema/迁移版本校验，任一步失败都保持只读且不得迁移。
3. 在应用迁移前写入 `schema_migrations(state = 'started', pre_migration_backup_id = <verified database_backups.id>)`，使失败记录也能按 migration version 定位 `backupId`；初始空库创建才允许该外键为空。
4. 在事务能力允许的范围内应用单个迁移并运行完整性检查。
5. 将同一 `schema_migrations` 记录更新为 `applied` 后再开放业务写入；失败时保留 migration 记录和其数据库备份，不进入自动清理。
...
1. 为全部目标计算排序稳定的 `canonical_target_key`，在同一 `BEGIN IMMEDIATE` 中先确认没有 unresolved `recovery_locks`，再取得共享 `deployment_locks` 租约及单调 `fence_token`；存在恢复锁时拒绝正常部署。随后创建 `deployment`、顺序化的 `deployment_operations` 和待验证备份记录。
...
6. 任一步失败时按 `sequence_no` 逆序执行补偿；补偿前同样验证 fencing token，补偿结果逐步持久化，崩溃后由新 owner 取得更高 token 后恢复继续。若不能自动补偿，必须先在 `BEGIN IMMEDIATE` 中为每个相关 key 插入或保留 unresolved `recovery_locks`，再以 owner/token 条件释放短期 deployment lease；进入人工恢复不能删除持久恢复锁。

数据库事务不得在慢速文件 I/O 期间长时间保持；长操作由 owner 在 `BEGIN IMMEDIATE` 短事务中续租。SQLite 租约只能协调 AI Config Hub 的 desktop/CLI 实例，不能阻止编辑器或其他外部程序修改目标，因此每次实际写入前仍须复核源/目标哈希和打开句柄的 file identity。若文件已写而状态尚未提交即崩溃，恢复器通过 fencing token、目标哈希、文件 identity、临时文件、备份哈希和操作状态判定继续、补记或回滚；不凭时间戳猜测。补偿失败时 `DeploymentRecord.status` 保持 `failed`，系统为相关 `canonical_target_key` 写入 unresolved `recovery_locks`（可对用户显示 `recovery_required`），保留所有证据并阻止同一路径的新正常部署。短期 `deployment_locks` 可在进入人工恢复后释放；持久恢复锁只能由受控恢复用例 claim，并在恢复写入和全量验证成功后以 resolution evidence 显式解除。`recovery_required` 不是 `DeploymentRecord.status`。
...
- `tools`、`projects`、`scopes`、`assets`、`asset_references` 和派生诊断可通过全量扫描重建；清库不会修改配置文件。`settings`、任务/部署审计、备份关联、迁移记录和 recovery lock 不能靠重扫恢复。
- 诊断和失败扫描按保留期清理，但部署审计、unresolved `recovery_locks` 及其关联 deployment、仍可回滚的配置备份不得被孤立删除。
- 配置备份到期清理先验证部署状态和恢复策略，再安全删除文件，最后删除元数据；失败时保留元数据并重试。
- `database_backups` 使用独立保留策略；任何被 `schema_migrations.pre_migration_backup_id` 引用、关联失败迁移、处于当前升级/回退窗口或作为最近已验证恢复点的记录均不得自动清理。
...
- 每个现有库 migration 都能通过 `schema_migrations.pre_migration_backup_id` 定位已验证 `database_backups.backupId`；缺失时不执行迁移。
- 文件写入失败能按补偿日志逆序恢复，崩溃后可继续恢复。
- 自动补偿失败会在释放 deployment lease 前创建持久 `recovery_locks`；普通部署无法绕过，只有受控恢复验证后才能显式解除。
