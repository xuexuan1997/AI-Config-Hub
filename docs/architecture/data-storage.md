...
- 业务主键使用随机 UUID，数据库类型为 `TEXT`；主键不由路径哈希或内容哈希生成。
...
| --- | --- |
| 主键 | `id TEXT` |
| 核心字段 | `tool_key`、`display_name`、`detected_version`、`adapter_version`、`capabilities_json`、`last_seen_at` |
| 唯一约束 | `UNIQUE(tool_key)` |
| 删除行为 | 被 `scopes`、`assets` 或历史部署引用时禁止删除；工具消失使用软状态 `is_detected = 0` |
| 关键索引 | `idx_tools_detected(is_detected, tool_key)` |

`tool_key` 是稳定代码标识，例如 `claude-code`、`cursor`、`codex`、`opencode`。
...
| --- | --- |
| 主键 | `id TEXT` |
| 核心字段 | `root_path_display`、`root_path_normalized`、`name`、`git_root_normalized`、`first_seen_at`、`last_seen_at` |
| 唯一约束 | `UNIQUE(root_path_normalized)` |
...
| --- | --- |
| 主键 | `id TEXT` |
| 外键 | `tool_id → tools.id ON DELETE RESTRICT`；`project_id → projects.id ON DELETE CASCADE`，用户级作用域允许为空；`parent_scope_id → scopes.id ON DELETE CASCADE` |
| 核心字段 | `scope_kind`、`root_path_display`、`root_path_normalized`、`depth`、`precedence`、`adapter_scope_key` |
| 唯一约束 | `UNIQUE(tool_id, project_id, root_path_normalized, adapter_scope_key)`；SQLite 中对空 `project_id` 使用表达式唯一索引统一为空哨兵 |
...
| --- | --- |
| 主键 | `id TEXT` |
| 外键 | `tool_id → tools.id ON DELETE RESTRICT`；`scope_id → scopes.id ON DELETE CASCADE`；`last_scan_run_id → scan_runs.id ON DELETE SET NULL` |
| 核心字段 | `resource_type`、`logical_key`、`source_path_display`、`source_path_normalized`、`content_hash`、`observed_mtime_ms`、`observed_size`、`normalized_json`、`normalized_schema_version`、`adapter_version`、`parse_status`、`sensitive_summary_json`、`first_seen_at`、`last_seen_at` |
| 唯一约束 | `UNIQUE(tool_id, scope_id, source_path_normalized, logical_key)` |
...
| 项目 | 定义 |
| --- | --- |
| 主键 | `id TEXT`，同时作为公开 `taskId` |
| 外键 | `project_id → projects.id ON DELETE SET NULL` |
| 核心字段 | `scan_kind`、`status`、`phase`、`requested_roots_json`、`started_at`、`finished_at`、`discovered_count`、`processed_count`、`succeeded_count`、`failed_count`、`cancel_requested_at`、`error_summary_json` |
| 唯一约束 | 无；每次请求均生成独立记录 |
| 关键索引 | `idx_scan_runs_status(status, started_at DESC)`、`idx_scan_runs_project(project_id, started_at DESC)` |

任务记录是审计和重连依据。成功提交索引后才将状态改为 `succeeded` 或 `partial`。

### 3.8 `deployments`

| 项目 | 定义 |
| --- | --- |
| 主键 | `id TEXT`，同时作为部署 `taskId` |
| 外键 | `project_id → projects.id ON DELETE SET NULL`；`source_asset_id → assets.id ON DELETE SET NULL`；`target_tool_id → tools.id ON DELETE RESTRICT` |
| 核心字段 | `preview_id`、`state`、`source_hash`、`target_hash_before`、`plan_json`、`compatibility`、`requested_at`、`confirmed_at`、`finished_at`、`verification_json`、`rollback_state`、`correlation_id` |
| 唯一约束 | `UNIQUE(preview_id)`，一个预览最多被执行一次 |
| 关键索引 | `idx_deployments_history(project_id, requested_at DESC)`、`idx_deployments_state(state, requested_at)`、`idx_deployments_target(target_tool_id, requested_at DESC)` |
...
| 核心字段 | `sequence_no`、`operation_kind`、`target_path_normalized`、`expected_hash_before`、`result_hash_after`、`state`、`compensation_kind`、`compensation_payload_json`、`started_at`、`finished_at`、`error_code` |
...
| 核心字段 | `backup_path_normalized`、`target_path_normalized`、`content_hash`、`size_bytes`、`created_at`、`expires_at`、`permission_mode`、`encryption_state`、`restore_verified_at` |
| 唯一约束 | `UNIQUE(operation_id)`，每个覆盖或删除操作必须恰有一个可验证备份 |
...
删除部署历史前必须先完成明确的备份保留或清理流程；外键使用 `RESTRICT` 防止审计记录与实际备份失配。
...
WAL 允许 UI/CLI 读查询与扫描写事务并发，但不代表允许多个写任务修改相同目标。应用仍对扫描提交、迁移和部署历史更新使用单写者队列；按规范化目标路径另加进程内互斥锁。定期 checkpoint 在空闲期执行，不能在部署原子写入窗口中强制阻塞。
...
全量扫描的替换范围是请求所覆盖的全部作用域；增量扫描只替换事件影响的路径及依赖闭包。解析失败的文件以 `parse_status = 'failed'` 的最小资产壳和诊断进入 staging，其他成功资产仍可提交，任务状态为 `partial`。取消发生在 `committing` 前时丢弃未提交 staging；一旦开始 `committing`，必须完成提交或让数据库事务整体回滚。

读查询永远只读当前已提交快照，不读取 staging。进程崩溃后，启动清理未完成 staging，并把对应 `scan_runs.status` 标记为 `interrupted`；旧索引仍可读。
...
2. 备份 SQLite 主文件、WAL 和 SHM 的一致快照。
...
1. 数据库事务创建 `deployment`、顺序化的 `deployment_operations` 和待验证备份记录。
2. 对每个目标先安全备份并 `fsync`，再把操作状态提交为 `backed_up`。
3. 在目标目录创建权限受限的临时文件，写入、`fsync`、解析验证后原子替换目标，并按平台能力同步父目录。
4. 每个文件操作完成后独立提交结果哈希和下一步补偿动作。
5. 全部文件完成后重新扫描并验证，最后标记部署成功。
6. 任一步失败时按 `sequence_no` 逆序执行补偿；补偿结果逐步持久化，崩溃后可恢复继续。

数据库事务不得在慢速文件 I/O 期间长时间保持。若文件已写而状态尚未提交即崩溃，恢复器通过目标哈希、临时文件、备份哈希和操作状态判定继续、补记或回滚；不凭时间戳猜测。补偿失败时部署进入 `recovery_required`，保留所有证据并阻止同一路径的新部署。
