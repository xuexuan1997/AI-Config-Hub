...
main process 仅注册版本化固定 channel，检查调用来源窗口和 frame，拒绝 subframe/未知 WebContents。请求和响应执行双端 Zod 校验，错误 envelope 脱敏。`migration.preview` 只向 renderer 返回不可变 `planId`/`planHash` 和差异，不返回执行凭据。main 通过原生确认 UI 重新展示持久计划；真实确认后只在 main 内存创建短时、单次、绑定 `planId + planHash + window session` 的 `ConfirmationGrant`。`deployment.execute` 的 IPC 请求只携带 `planId`，grant 永不暴露给 renderer、日志或 SQLite。CLI 在自身交互/`--yes` 边界创建等价的进程内 grant。
...
Pino 日志使用事件类型定义的 allowlist，而不是先记录整个对象再 redact。允许字段包括 `timestamp`、`level`、`eventCode`、`correlationId`、`taskId`、业务实体 ID、计数、phase、稳定错误码和缩略路径。禁止字段包括配置正文、任意请求对象、环境变量、命令行原串、Git credential、认证 URL、`ConfirmationGrant`、备份内容和 SQLite 行转储。
...
安全事件使用稳定 code 和 correlation ID 记录，不记录攻击 payload 原文。高优先级事件包括越界路径、symlink/file identity 变化、未知 IPC、缺失或重复消费内部 grant、陈旧预览、lease/fence 拒绝、备份哈希错误、补偿失败和应用签名/依赖校验失败。

最低测试集：

- 路径遍历、大小写、Unicode、UNC、设备路径、symlink/junction 与 TOCTOU 夹具；
- renderer 发起未知 channel、畸形 Zod payload、subframe 调用、绕过原生确认和伪造 `planId` 的 E2E 测试；测试必须证明 renderer 永远无法读取或提交执行 grant；
...
- renderer 只提交 `planId`，执行 grant 仅存在于 desktop main/CLI 进程且单次短时有效。
