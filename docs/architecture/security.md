...
| 符号链接逃逸 | 扫描根、配置文件、父目录、临时文件或备份路径被替换为 symlink/junction/reparse point | 越界读取、覆盖任意文件或 TOCTOU | 枚举和操作前解析 `realpath`；逐段检查链接；默认不跟随越根 symlink；写入前后校验父目录身份；目标目录内创建临时文件 | 比较 canonical path、文件标识和 `lstat`；发现改变返回 `SYMLINK_ESCAPE`/`STALE_TARGET` | 停止批次并回滚已完成操作；将路径锁定为 `recovery_required`，要求用户检查链接 |
| renderer compromise | XSS、恶意 Markdown/HTML、前端依赖漏洞、DevTools 滥用 | 调用高权限能力、窃取数据、发起部署 | `contextIsolation: true`、`nodeIntegration: false`、兼容时 `sandbox: true`；严格 CSP；最小 preload；固定 IPC channel；main 端重新授权；不渲染原始 HTML | CSP 报告、本地安全日志、未知 channel 拒绝计数、E2E 负向测试 | 关闭受影响窗口/应用，吊销会话确认令牌；检查部署历史和文件哈希；升级后重新扫描 |
...
| 陈旧预览覆盖 | 预览后源或目标被编辑、另一窗口/CLI 并发部署 | 覆盖新修改、生成不一致配置 | 预览绑定源/目标 SHA-256、目标 canonical path、过期时间和一次性确认令牌；目标路径互斥锁；写入前复核 | `STALE_PREVIEW`/`STALE_TARGET`；Chokidar 哈希漂移；部署 preflight 对账 | 不写入并要求重新预览；若写入中发现漂移，停止后续操作并按补偿日志回滚 |
...
main process 仅注册版本化固定 channel，检查调用来源窗口和 frame，拒绝 subframe/未知 WebContents。请求和响应执行双端 Zod 校验，错误 envelope 脱敏。部署确认令牌是短期、一次性、绑定 preview 和窗口会话的数据；renderer 无法自行构造或修改部署计划。
...
5. 通过 `lstat`/文件标识验证每个敏感操作前后的父目录和目标未被替换。
...
- 无法可靠取得文件身份或原子替换语义的文件系统进入只读扫描模式，禁止部署。

### 7.3 权限与原子写入

- 新配置和临时文件继承目标文件的安全权限；没有既有目标时使用用户专属默认权限，不能创建全局可写文件。
- 备份目录和数据库目录仅当前用户可读写；POSIX 目标通常为目录 `0700`、文件 `0600`，Windows 使用当前用户 ACL，不依赖 POSIX 数字在所有平台生效。
- 写入使用目标目录内随机临时文件，完成写入、`fsync`、重新解析和哈希后原子替换，并在平台支持时同步父目录。
- 批量部署逐项持久化补偿日志。任何失败按逆序恢复；补偿失败锁定目标并进入 `recovery_required`。
...
Pino 日志使用事件类型定义的 allowlist，而不是先记录整个对象再 redact。允许字段包括 `timestamp`、`level`、`eventCode`、`correlationId`、`taskId`、业务实体 ID、计数、phase、稳定错误码和缩略路径。禁止字段包括配置正文、任意请求对象、环境变量、命令行原串、Git credential、认证 URL、确认令牌、备份内容和 SQLite 行转储。
...
安全事件使用稳定 code 和 correlation ID 记录，不记录攻击 payload 原文。高优先级事件包括越界路径、symlink 身份变化、未知 IPC、确认令牌复用、陈旧预览、备份哈希错误、补偿失败和应用签名/依赖校验失败。

最低测试集：

- 路径遍历、大小写、Unicode、UNC、设备路径、symlink/junction 与 TOCTOU 夹具；
- renderer 发起未知 channel、畸形 Zod payload、subframe 调用和伪造确认令牌的 E2E 测试；
...
发现可能泄密时，先停止进一步日志/导出/同步并保全脱敏审计，再识别暴露副本、提示用户轮换相关凭据、清理副本并发布修复。发现未授权写入时，阻止同一路径后续部署，以备份和部署操作日志恢复，并在重新扫描验证前保持 `recovery_required`。
...
- canonical path、allowed roots、symlink 和文件身份校验覆盖读写及备份路径。
