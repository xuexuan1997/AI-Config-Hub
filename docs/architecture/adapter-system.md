...
- 实现工具自身的继承、优先级、合并、覆盖和忽略规则，解释指定目录下的 `EffectiveConfig`。
...
export type SecretValue =
  | { readonly kind: "reference"; readonly expression: string }
  | { readonly kind: "redacted"; readonly digest: ContentHash };
...
export interface DeploymentPlan {
  ...
  readonly conversionResultIds: readonly ConversionResultId[];
  readonly operations: readonly DeploymentOperation[];
  readonly diffs: readonly DeploymentDiff[];
  readonly expectedSourceHashes: Readonly<Record<AssetId, ContentHash>>;
  readonly backupPolicy: "required";
  readonly warningsAccepted: readonly string[];
...
- `planDeployment` 只生成确定性候选计划，不读写未提供的路径；实际备份、锁、哈希复检、原子写入和回滚由 `packages/deployer` 完成。
...
function validateAdapterRegistrations(
...
export function createAdapterRegistry(
  ...
  validateAdapterRegistrations(registrations); // validates before Map can overwrite
  return new Map(
...
应用启动时必须在接收请求前校验：`toolId` 和 `adapterId` 无重复；`contractVersion === 1`；注册元数据与实例属性一致；`resourceKinds` 与转换声明只包含统一联合成员；`testedToolVersions` 都落在 `supportedToolVersions` 内；`writtenSchemaVersion` 可被当前核心读取。任一校验失败都视为构建/启动错误，不在运行中悄悄覆盖旧注册。
...
| --- | --- | --- | --- | --- | --- | --- |
| Claude Code | 发现/解析四类资源；黄金规范化与生效结果确定 | 定位行列/字段；单文件 `rejected`，批次部分成功 | 用户、项目、子目录优先级及覆盖步骤正确 | 保留在 `extensions` 或给出明确诊断，不静默提升语义 | Token/env/header 在结果、日志、快照中均不出现明文 | 下界、上界、范围内未测试、上界后新版本诊断 |
| Cursor | 发现/解析四类资源；黄金规范化与生效结果确定 | 定位行列/字段；单文件 `rejected`，批次部分成功 | 用户、项目、子目录优先级及覆盖步骤正确 | 保留在 `extensions` 或给出明确诊断，不静默提升语义 | Token/env/header 在结果、日志、快照中均不出现明文 | 下界、上界、范围内未测试、上界后新版本诊断 |
| Codex | 发现/解析四类资源；黄金规范化与生效结果确定 | 定位行列/字段；单文件 `rejected`，批次部分成功 | 用户、项目、子目录优先级及覆盖步骤正确 | 保留在 `extensions` 或给出明确诊断，不静默提升语义 | Token/env/header 在结果、日志、快照中均不出现明文 | 下界、上界、范围内未测试、上界后新版本诊断 |
| OpenCode | 发现/解析四类资源；黄金规范化与生效结果确定 | 定位行列/字段；单文件 `rejected`，批次部分成功 | 用户、项目、子目录优先级及覆盖步骤正确 | 保留在 `extensions` 或给出明确诊断，不静默提升语义 | Token/env/header 在结果、日志、快照中均不出现明文 | 下界、上界、范围内未测试、上界后新版本诊断 |
...
| 部署计划 | 输出路径在目标根内；操作顺序和 `planHash` 确定；输入哈希变化使旧计划无效 |
